...
| 「テンプレート同期して」「更新して」 | プラグインの最新テンプレートと `.engineer/` 内のファイルを比較し、差分をマージ |

**チームメンバーの視点は autodev 以外でも自然に出す。** 詳細は `team/lead.md` の「チームメンバーの活用」セクションを参照。

### テンプレート同期

ユーザーが「テンプレート同期して」「更新して」と言った場合に実行する。**起動時には自動実行しない。**

プラグインの `references/` と `.engineer/` 内のファイルを比較し、差分をリストアップする。

**同期対象:**

| ファイル | 同期方法 |
|---------|---------|
| `team/lead.md` | テンプレートの新セクションを**マージ**（ユーザーのカスタマイズを保持） |
| `team/guardian.md` | 新セクション（activation 等）を**追記**。既存内容は保持 |
| `config.yml` | 新しいキー（adapters 等）を**追記**。既存値は変更しない |

**ユーザーのカスタマイズは消さない。** 新セクションの追加・新キーの追記のみ行う。
